...
namespace Lab06.Tools3D.Transformation
{
    class Gizmo
    {
        ...
        {
            xView = Axis(Constants.xColor);
            yView = Axis(Constants.yColor);
            zView = Axis(Constants.zColor);
        ...
        private Group Axis(Color color)
        {
            Base3D.Group arrow = new Group();
            arrow.Matreial = new SolidMaterial(color);
            ...
            spline.Matreial = new SolidMaterial();
            spline.Add(new Base3D.Point { });
            spline.Add(new Base3D.Point { X = 20 });
            arrow.Add(spline);
            arrow.Apply(Matrix.Move(new Base3D.Point { X = -10 }));
        ...
        {
            var (rotateMatrix, movingMatrix) = RotateMatrix(angleX, angleY, angleZ);
        ...
        public (Matrix rotateMatrix, Matrix movingMatrix) RotateMatrix(double angleX, double angleY, double angleZ)
        {
            ...
            Matrix movingMatrix = Matrix.Move(-location) *
                                  rotateMatrix *
                                  Matrix.Move(location);
            return (rotateMatrix, movingMatrix);
        }
